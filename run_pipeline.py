...
def stage_2_descriptive() -> None:
    ...
    _section(2, "Descriptive statistics (Blueprint v4 §8)")
...
def stage_5_global_shap(trained_models: dict, test: pd.DataFrame) -> dict:
    ...
    _section(5, "Global SHAP analysis (Blueprint v4 §10.2)")
...
def stage_6_nonlinear_shap(shap_artifacts: dict, test: pd.DataFrame) -> None:
    ...
    _section(6, "Non-linear SHAP effects (Blueprint v4 §10.3)")
...
def stage_7_local_shap(trained_models: dict, test: pd.DataFrame) -> None:
    ...
    _section(7, "Local SHAP explanations (Blueprint v4 §10.4)")
...
def stage_8_theory(shap_artifacts: dict, test: pd.DataFrame) -> None:
    ...
    _section(8, "Theory-consistency validation table (Blueprint v4 §10.5)")
...
def stage_9_robustness(
    ...
    _section(9, "Robustness checks RC1–RC5 (Blueprint v4 §11)")
...
def main(args: argparse.Namespace) -> None:
    ...
    # Transparent, pre-specified EXTENSION (not a frozen Blueprint RC). Opt-in
