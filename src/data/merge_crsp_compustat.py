...
=================================================================
Implements the two-layer CCM + CUSIP merge (design §6.2) and
the leakage-free distress label construction (design §5.2).
...
def attach_filing_dates(
    ...
    F1 fix — filing-date rule from design §5.2:
...
def merge_ccm_primary(
    ...
    linktype IN ('LC','LU') and linkprim IN ('P','C') per the pre-specified study design
...
def validate_cusip(
    ...
    """
    Cross-validate the CCM merge using CUSIP codes (design §6.2).
...
def apply_missingness_filter(
    ...
    F4 fix — design §6.4 states that the strict ≥8-of-11
