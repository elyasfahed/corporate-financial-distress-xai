...
def apply_post_winsor_missingness_filter(
    ...
    applied *after winsorisation* (Blueprint v4 §6.4).
...
def split_panel(panel: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    Split boundaries (Blueprint v4 §3.4, frozen):
...
def run_feature_pipeline(
    ...
    # Blueprint v4 §6.4 says the strict missingness filter is applied AFTER
